export function formatDate(
    ...
    if (diffMinutes < 60) return `${stringLabel} Hace ${diffMinutes} minuto`;
    if (diffHours < 24) return `${stringLabel} Hace ${diffHours} hora`;
    if (diffDays < 7) return `${stringLabel} Hace ${diffDays} dia`;
