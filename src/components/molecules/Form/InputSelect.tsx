...
export const InputSelect = ({ label, name, register, errors, className = "", children, setValue, onSearch, readOnly = false, inputValue = false }: InputTextProps & {
    readOnly?: boolean, className?: string, children?: React.ReactNode, setValue: UseFormSetValue<any>, inputValue?: boolean
}) => {
    ...
    return (
        <div className={`h-auto ${className} relative`}>
            <div className="h-10 flex flex-col relative">
                <input id={name} type="text" value={marc.search} onKeyDown={(e) => readOnly && e.preventDefault()} onFocus={() => setList(true)}  {...register(name, {
                    onChange: (e) => {
                        setList(true)
            ...
            {children && list && <div onMouseDown={(e) => e.preventDefault()} className="absolute translate-y-1 transition-all text-sm rounded-xl w-full flex flex-col z-2 px-1 py-1 bg-white border border-gray-200" role="listbox">{childrenArray.map((val, index) => {
                ...
                return dataGroup ? <React.Fragment key={index}>
                    {val}
                </React.Fragment> : filterSearch(marc.filter, label) && <div key={index} onClick={(e) => {
                    setMarc({ id, label, search: label, filter: "" })
                    setValue(name, id, { shouldValidate: true })
                    setList(false)

                    console.log(marc)
                }} data-id={id} id="option-box" role="option" className={`px-2 py-1.5 cursor-pointer ${marc.id === id ? 'bg-blue-50' : ''} hover:bg-gray-100 rounded-md `}>{val}</div>
            })}</div>}
            {error && (
